...
* ``%gs``: always holds the sandbox base address.
...
runtime call entrypoint table. The `%r11` register stores the return address.
  ...
  The location of the runtime call entrypoint table is the first page of the
  sandbox. For the final version of LFI merged into LLVM we intend to move this
  location to the page before the first page of the sandbox, with entrypoints
  starting at the largest addresses in that page. As a result, x86-64 system
  calls will use ``-8(%r14)`` instead of ``0(%r14)``.
...
TLS accesses are rewritten into runtime calls to the second entry in the
runtime call entrypoint table. ``%rax`` will contain the return value, so it
must be swapped when targeting another register.
...
``-mno-tls-direct-seg-refs``.
  ...
  More complicated rewrite sequences can be used to support non-``%fs:0``
  accesses.
